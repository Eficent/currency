...
class ResCurrencyRateProviderCMC(models.Model):
    ...
    service = fields.Selection(
        selection_add=[('CMC', 'Coin Market Cap')],
    )
    ...
    def _obtain_rates(self, base_currency, currencies, date_from,
        ...
        if base_currency not in self.allowed_base_currencies:
        ...
        if not vals:
            raise Exception('Could not fetch values for main currency '
                            '%s.' % base_currency)
        for val in vals:
            for quote in val['data']['quotes']:
                date_content = content[quote['timestamp'][:10].isoformat()]
                if 'quote' in quote:
                    if base_currency not in quote['quote']:
                        raise Exception('Attribute %s not found.' %
                                        base_currency)
                    for currency, rate in quote['quote'].items():
                        date_content[currency] = rate['price']
        return content
